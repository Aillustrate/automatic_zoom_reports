...
def validate_structured_summary(summary) -> bool:
    ...
    for i, item in enumerate(summary):
        ...
        if not isinstance(item["topic"], str):
            return (
                False,
                f"Summary item {i} has 'topic' key with {type(item['topic'])} value, expected str",
            )
        if item["topic"] == "":
        ...
        if not isinstance(item["points"], list):
            return (
                False,
                f"Summary item {i} has 'points' key with {type(item['points'])} value, expected List",
            )
        for j, point in enumerate(item["points"]):
            if not isinstance(point, str):
                return (
                    False,
                    f"Summary item {i} has 'points' key with {type(point)} value at {j} position, expected str",
                )
            if point == "":
                return (
                    False,
                    f"Summary item {i} has empty string at {j} position in 'points' key",
                )
